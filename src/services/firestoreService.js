...
const normalizarEmailRanking = (valor = "") =>
  String(valor || "").trim().toLowerCase();

const timestampParaMillis = (valor) => {
  if (!valor) return 0;
...
const normalizarRankingComunidade = (perfil = {}) => {
  ...
  const email = normalizarEmailRanking(perfil.email);
  return {
    ...perfil,
    email,
...
const escolherPerfilComunidade = (atual, candidato) => {
  if (!atual) return candidato;

  const scoreAtual = Number(atual.score || atual.pontuacao || 0);
  const scoreCandidato = Number(candidato.score || candidato.pontuacao || 0);
  if (scoreCandidato !== scoreAtual) {
    return scoreCandidato > scoreAtual ? candidato : atual;
  }

  const dataAtual = timestampParaMillis(atual.updatedAt || atual.lastUpdated);
  const dataCandidato = timestampParaMillis(candidato.updatedAt || candidato.lastUpdated);
  if (dataCandidato !== dataAtual) {
    return dataCandidato > dataAtual ? candidato : atual;
  }

  if (!atual.uid && candidato.uid) return candidato;
  return atual;
};

const deduplicarRankingComunidade = (lista = []) => {
  const porUid = new Map();
  const semUid = [];

  lista.forEach((perfil) => {
    const normalizado = normalizarRankingComunidade(perfil);
    const uid = String(normalizado.uid || normalizado.id || "").trim();
    if (!uid) {
      semUid.push(normalizado);
      return;
    }

    porUid.set(uid, escolherPerfilComunidade(porUid.get(uid), normalizado));
  });

  const porEmail = new Map();
  [...porUid.values(), ...semUid].forEach((perfil) => {
    const email = normalizarEmailRanking(perfil.email);
    if (!email) {
      porEmail.set(`sem-email:${perfil.uid || perfil.id || perfil.nome}`, perfil);
      return;
    }

    porEmail.set(email, escolherPerfilComunidade(porEmail.get(email), perfil));
  });
...
const normalizarMissoes = (missions = []) =>
  missions
    .map((mission) => {
      if (mission.id === "mapear_cinco_aparelhos") {
        return {
          ...mission,
      ...
      if (mission.id === "chuveiro_modo_verao") {
        return {
          ...mission,
          scoreNecessario: Math.max(Number(mission.scoreNecessario || 0), 100),
          ordem: Math.max(Number(mission.ordem || 0), 3),
          bloqueada: true,
        };
      }
...
const montarCommunityProfile = (perfil) => {
  ...
  return {
    uid: perfil.uid,
    nome: perfil.nome || "Participante EnergiaPI",
    email: normalizarEmailRanking(perfil.email),
...
export const completeUserOnboarding = async (firebaseUser, perfil) => {
  ...
  if (payload.email) {
    const duplicados = await getDocs(
      query(colecao("community"), where("email", "==", payload.email)),
    );
    await Promise.all(
      duplicados.docs
...
export const subscribeRankings = (callback) => {
  ...
  const state = {
    ...
    comunidade: cached?.comunidade
      ? ordenarRankingComunidade(deduplicarRankingComunidade(cached.comunidade))
      : null,
    escolasReady: Boolean(cached?.escolas),
    comunidadeReady: Boolean(cached?.comunidade),
  };
  ...
  if (cached) {
    callback({
      ...
      loading: !(state.escolasReady && state.comunidadeReady),
      fromCache: true,
    });
  }
  ...
  const unsubscribeEscolas = onSnapshot(
    ...
    (error) => {
      ...
      state.escolasReady = true;
      emitir({
        error: "Não foi possível atualizar os rankings agora.",
  ...
  Promise.allSettled([listarRankingEscolas(), listarRankingComunidade()]).then(
    (results) => {
      ...
      if (escolasResult.status === "fulfilled") state.escolasReady = true;
      if (comunidadeResult.status === "fulfilled") state.comunidadeReady = true;
      if (
